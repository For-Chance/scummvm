...
#include "graphics/tinygl/tinygl.h"	// TODO: remove this include
#include "math/glmath.h"

#include "freescape/objects/object.h"
#include "freescape/gfx_fitd.h"
#include "freescape/gfx_fitd_texture.h"

namespace Freescape {

Renderer *CreateGfxFitd(int screenW, int screenH, Common::RenderMode renderMode) {
	return new FitdRenderer(screenW, screenH, renderMode);
}

FitdRenderer::FitdRenderer(int screenW, int screenH, Common::RenderMode renderMode) : Renderer(screenW, screenH, renderMode, true) {
...
FitdRenderer::~FitdRenderer() {
	for (auto &it : _stippleTextureCache) {
		delete (Fitd3DTexture *)it._value;
	}
...
Texture *FitdRenderer::createTexture(const Graphics::Surface *surface, bool is3D) {
	if (is3D)
		return new Fitd3DTexture(surface);
	else
		return new Fitd2DTexture(surface);
}

void FitdRenderer::freeTexture(Texture *texture) {
	//FitdTexture *glTexture = static_cast<FitdTexture *>(texture);
	delete texture;
}

void FitdRenderer::init() {
...
void FitdRenderer::setViewport(const Common::Rect &rect) {
	_viewport = rect;
	tglViewport(rect.left, g_system->getHeight() - rect.bottom, rect.width(), rect.height());
}

void FitdRenderer::drawTexturedRect2D(const Common::Rect &screenRect, const Common::Rect &textureRect, Texture *texture) {
	...
	tglBlit(((Fitd2DTexture *)texture)->getBlitTexture(), transform);
}

void FitdRenderer::drawSkybox(Texture *texture, Math::Vector3d camera) {
	Fitd3DTexture *glTexture = static_cast<Fitd3DTexture *>(texture);
...
void FitdRenderer::updateProjectionMatrix(float fov, float aspectRatio, float nearClipPlane, float farClipPlane) {
	tglMatrixMode(TGL_PROJECTION);
	tglLoadIdentity();
...
void FitdRenderer::positionCamera(const Math::Vector3d &pos, const Math::Vector3d &interest) {
...
void FitdRenderer::renderSensorShoot(byte color, const Math::Vector3d sensor, const Math::Vector3d player, const Common::Rect &viewArea) {
	tglEnable(TGL_BLEND);
...
void FitdRenderer::renderPlayerShootBall(byte color, const Common::Point &position, int frame, const Common::Rect &viewArea) {
	/*uint8 r, g, b;
...
void FitdRenderer::renderPlayerShootRay(byte color, const Common::Point &position, const Common::Rect &viewArea) {
	uint8 r, g, b;
...
void FitdRenderer::renderCrossair(const Common::Point &crossairPosition) {
	tglMatrixMode(TGL_PROJECTION);
	tglLoadIdentity();
	tglOrtho(0, _screenW, _screenH, 0, 0, 1);
	tglMatrixMode(TGL_MODELVIEW);
...
void FitdRenderer::setStippleData(byte *data) {
	if (!data) {
		_stippleTexture = nullptr;
...
void FitdRenderer::useStipple(bool enabled) {
...
void FitdRenderer::renderFace(const Common::Array<Math::Vector3d> &vertices) {
	...
	if (_stippleEnabled) {
		if (_stippleTextureCache.contains(uint64(_variableStippleArray))) {
			...
			_stippleTexture = new Fitd3DTexture(_variableStippleArray, _lastColorSet0, _lastColorSet1);
...
void FitdRenderer::drawCelestialBody(Math::Vector3d position, float radius, byte color) {
...
void FitdRenderer::depthTesting(bool enabled) {
	if (enabled) {
...
void FitdRenderer::polygonOffset(bool enabled) {
	if (enabled) {
...
void FitdRenderer::useColor(uint8 r, uint8 g, uint8 b) {
	_lastColorSet1 = _lastColorSet0;
	_lastColorSet0 = _texturePixelFormat.RGBToColor(r, g, b);
	tglColor3ub(r, g, b);
}

void FitdRenderer::clear(uint8 r, uint8 g, uint8 b, bool ignoreViewport) {
...
void FitdRenderer::drawFloor(uint8 color) {
	uint8 r1, g1, b1, r2, g2, b2;
	byte *stipple = nullptr;
...
Graphics::Surface *FitdRenderer::getScreenshot() {
	Graphics::Surface glBuffer;
	TinyGL::getSurfaceRef(glBuffer);
