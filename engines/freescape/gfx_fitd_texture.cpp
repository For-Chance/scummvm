 ...
 */
 ...
 #include "graphics/tinygl/zblit.h"	// TODO: remove this include

 #include "freescape/gfx_fitd_texture.h"

 namespace Freescape {

 Fitd2DTexture::Fitd2DTexture(const Graphics::Surface *surface) {
	 _width = surface->w;
	 _height = surface->h;
 ...
 Fitd2DTexture::~Fitd2DTexture() {
	 tglDeleteBlitImage(_blitImage);
 }

 void Fitd2DTexture::update(const Graphics::Surface *surface) {
	 uint32 keyColor = getRGBAPixelFormat().RGBToColor(0xA0, 0xA0, 0xA0);
	 tglUploadBlitImage(_blitImage, *surface, keyColor, true);
 }

 void Fitd2DTexture::updatePartial(const Graphics::Surface *surface, const Common::Rect &rect) {
	 // FIXME: TinyGL does not support partial texture update
	 update(surface);
 }

 TinyGL::BlitImage *Fitd2DTexture::getBlitTexture() const {
	 return _blitImage;
 }

 Fitd3DTexture::Fitd3DTexture(byte *stipple, uint32 c1, uint32 c2) {
 ...
 Fitd3DTexture::Fitd3DTexture(const Graphics::Surface *surface) {
	 _width = surface->w;
	 _height = surface->h;
	 _format = surface->format;
	 _upsideDown = false;
 ...
 Fitd3DTexture::~Fitd3DTexture() {
	 tglDeleteTextures(1, &_id);
 }

 void Fitd3DTexture::update(const Graphics::Surface *surface) {
 ...
 void Fitd3DTexture::updatePartial(const Graphics::Surface *surface, const Common::Rect &rect) {
	 // FIXME: TinyGL does not support partial texture update
	 update(surface);
 }
